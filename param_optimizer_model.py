...
def general_case(score_df: pd.DataFrame, set_inputs: Dict, score_column: str, parameter_columns: List[str], iterations: int) -> pd.DataFrame:
    ...
    train_pool = Pool(data=fit_df[updated_parameter_columns], label=fit_df[score_column],
                      # group_id=fit_df['group_id'],
                      cat_features=categorical_cols)

    # model = CatBoostRanker(iterations=100, learning_rate=0.1, depth=12, loss_function='YetiRank')
    model = CatBoostRegressor(iterations=100, learning_rate=0.1, depth=12)
    ...
    synthetic_data = generate_interactions(synthetic_data, numeric_cols)
    # extended_df['group_id'] = 0

    extended_pool = Pool(data=synthetic_data[updated_parameter_columns], cat_features=categorical_cols)
    synthetic_data['predicted_score'] = model.predict(extended_pool)
    sorted_df = synthetic_data.sort_values(by='predicted_score')

    sorted_df = sorted_df[parameter_columns + ['predicted_score']]
    return sorted_df
